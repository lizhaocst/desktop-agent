...
## 本轮完成进度（frontend）
- 状态：`R-P1-Loop-FE-v2` 已通过 `dev/test` 验收（PASS）。
- 验收 SHA：`23d22c8`
- 已完成模块：
  - F1: 已完成
  - F2: 已完成
  ...
  - F4: 已完成
- 最新修改记录（简要）：
  - `e46d1b4`：修复 IME 组合输入误发送；修复 `chat:stream` 订阅参数；补齐 `done` 无 delta 兜底文案与 streaming 超时退避。
...
- UI 变更不得依赖未定义的临时后端字段。
- 每次完成一个模块或提交一次功能修改后，必须在 `CLAUDE.md` 增加一条简要说明（SHA + 改动要点）。
